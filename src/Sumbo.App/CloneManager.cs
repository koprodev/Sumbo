...
/// <summary>
/// The app-level service hub: owns the process-wide <see cref="HotkeyHost"/>, the shared region/profile stores,
/// the persisted <see cref="Settings"/> and the <see cref="LocalizationCatalog"/>. <see cref="MainWindow"/> hosts
/// the one embedded mirror (<see cref="MirrorSurface"/>) and owns every hotkey's meaning — the manager just raises
/// <see cref="HotkeyRouted"/>/<see cref="SurfaceRequested"/> so tray/hotkey intent reaches the window without the
/// manager knowing any UI.
/// </summary>
...
public sealed class CloneManager : IDisposable
{
    ...
    /// <summary>
    /// Raised when the user asks to quit the whole application (tray Exit). The host closes the main window and
    /// exits the message loop — the one funnel for exit.
    /// </summary>
    public event EventHandler? ExitRequested;

    /// <summary>
    /// Raised for every global hotkey press. <see cref="MainWindow"/> — which hosts the embedded mirror — owns the
    /// action's meaning, including when no mirror is running.
    /// </summary>
    public event EventHandler<HotkeyAction>? HotkeyRouted;

    /// <summary>
    /// Raised for tray surface actions (show/hide, double-click restore, settings). The manager stays UI-agnostic:
    /// <see cref="TrayHost"/> calls <see cref="RequestSurface"/>, <see cref="MainWindow"/> subscribes.
    /// </summary>
    public event EventHandler<SurfaceRequest>? SurfaceRequested;

    /// <summary>Routes a tray surface action to the main window.</summary>
    public void RequestSurface(SurfaceRequest request) => SurfaceRequested?.Invoke(this, request);

    /// <summary>
    /// Raised when the main window first retires to the tray on close, so the tray icon can show the one-time
    /// residency balloon. Mirror of <see cref="SurfaceRequested"/> in the opposite direction — the window decides
    /// <em>when</em>, <see cref="TrayHost"/> owns the <c>NotifyIcon</c> that shows it.
    /// </summary>
    public event EventHandler? TrayResidencyNoticeRequested;

    /// <summary>Asks the tray icon to show the one-time tray-residency balloon.</summary>
    ...
    /// <summary>Current global settings. The tray/settings-panel toggles mutate this via the setters below.</summary>
    public Settings Current => _settings;

    /// <summary>The shared UI string catalog. The tray and settings panel read + subscribe to it.</summary>
    public LocalizationCatalog Localization => _localization;

    /// <summary>Shared saved-region store; the region panel edits it through the main-window shell.</summary>
    internal RegionStore Regions => _regionStore;

    /// <summary>Shared profile store; the profiles panel edits it through the main-window shell.</summary>
    internal ProfileService Profiles => _profiles;

    /// <summary>True when minimizing should hide to the tray instead of the taskbar. Consumed by
    /// <see cref="MainWindow"/>'s minimize-transition branch.</summary>
    public bool MinimizeToTray => _settings.MinimizeToTray;

    /// <summary>
    /// Toggles launch-at-startup. The Registry write happens first; the setting is persisted only on
    /// success, so a policy-blocked Registry leaves settings (and the tray checkbox) unchanged.
    ...
    /// <summary>Toggles the minimize-to-tray preference and persists it.</summary>
    ...
    /// <summary>
    /// Requests full application exit. Raises <see cref="ExitRequested"/> so the host can close the
    ...
    private void OnHotkeyPressed(object? sender, HotkeyAction action)
        // Every hotkey routes to the main window, which owns the embedded mirror.
...
/// <summary>Main-window surface actions routed from the tray to the single window:
/// ToggleVisible toggles minimize/restore (same meaning as the ToggleVisible hotkey), Restore (double-click)
/// restores only, OpenSettings restores + switches to the embedded settings panel.</summary>
