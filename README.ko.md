...
- **오버레이 모드(UI 숨김)** — 창 테두리·UI 를 감추고 미러 내용만 표시.
  오버레이 창은 크롭 모양을 그대로 따라가며(레터박스 없음), 다시 숨기면 마지막
  크기·위치로 복원.
- **트레이 상주** — 닫기(X)는 트레이로 숨김(설정에서 끄면 X = 즉시 종료), 종료는
  트레이 메뉴에서. Windows 시작 시 자동 실행 옵션.
- **18개 언어 UI** 실시간 전환 — 한국어·English·日本語·简体中文·Español·Deutsch·
  Français·Português·Русский·Italiano·Bahasa Indonesia·Tiếng Việt·Türkçe·Polski·
  Nederlands·Українська·Ελληνικά·Filipino.
...
**Q. 브라우저(크롬·엣지·웨일 등) 영상을 미러링하는데, 브라우저가 다른 창에
가려지면 영상이 멈춰요.**

크로미움 계열 브라우저의 절전 최적화(occlusion detection) 때문입니다. 브라우저
창이 다른 불투명 창에 **완전히** 가려지면 브라우저 스스로 렌더링을 중단하므로,
원본을 그대로 비추는 숨보 미러도 함께 멈춥니다(숨보는 원본 창에 어떤 개입도
하지 않습니다). 앞에 띄운 프로그램에 따라 증상이 다르게 보이는 것도 이 때문입니다
— 투명·레이어드 창은 "가림"으로 계산되지 않습니다.

해결: 브라우저 바로가기 속성 → **대상** 끝에 한 칸 띄고 아래 플래그를 붙인 뒤,
브라우저를 완전히 종료하고 다시 실행하세요.
